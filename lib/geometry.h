...
public:
	...
	//two end points
	Pointbase* endPoint(int i) const {
		return _endp[i];
	}
	Type type() const {
		return _type;
...
public:
	...
	//input functions
	void addPoint(unsigned int i, double x, double y, unsigned int countour);
	void addEdges();
	void initializate();
	...
	void triangulation();
	void delTriangulation() {
		std::cout << "not available for this version\n";
	}
...
private:
	...
	std::fstream _logfile;                            //log file for debug purpose;
};
